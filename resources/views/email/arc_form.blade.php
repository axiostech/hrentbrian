...
-->
            ...
            <tr>
              <td style="padding: 10px 20px 10px 30px;" valign="top" bgcolor="#ffffff">
                ...
                <p><span style="font-size: 14px;">Thank you for using Happy Rent Auto Rental Collection (ARC) system.</span></p>
                <p><span style="font-size: 14px;">Please click the following link to approve</span></p>
                <p>
                ...
                <p><span style="font-size: 14px;">Unit Details:</span></p>
                <p>
                  <span style="font-size: 14px;">
                    @if($arc->tenancy->room_name)
                      ({{$arc->tenancy->room_name}})
                    @endif
                    {{$arc->tenancy->unit->block_number}}
                    @if($arc->tenancy->unit->block_number)
                    <span>
                      ,
                    </span>
                    @endif
                    {{$arc->tenancy->unit->unit_number}}
                    @if($arc->tenancy->unit->address)
                    <span>
                      , {{$arc->tenancy->unit->address}}
                    </span>
                    @endif
                  </span>
                </p>
                <p><span style="font-size: 14px;">Rental Details:</span></p>
                <p>
                  <span style="font-size: 14px;">
                    RM {{$arc->amount}}
                  </span>
            ...
            <tr>
              <td style="line-height:1.5; font-size: 12px;" align="center" bgcolor="#ffffff">{{$arc->profile->name}}<br>
