...
class Billplz extends Model
    {
        ...
        //public static $api_key = 'cef82833-8a2f-4b5a-ac6c-51e0b6ab2e68';
        //public static $x_signature = 'S--ip8tcwsiFhvV8xvVd_JPg';
        public static $api_key = '';
        public static $x_signature = '';

        /*Production Happyrent*/
/*        public static $api_key = '06837f4d-d503-4fbb-a723-3de87e2404bd';
        public static $x_signature = 'S-0RmNWu62rp9q7jvC3fmoLw';*/

      /*Production Multi Connect Solutions (MSC) */
/*        public static $api_key = '6161a25f-ce76-4054-98fc-c3eeb7a9e464';
        public static $x_signature = 'S-LvZoFrPdHj6rmU_K-n1mKg';*/

       public function __construct($api_key, $signature)
        {
            $api_key = $api_key;
        ...
        public function create_collection($title = 'Payment For Rent', $mode = '')
        {
      ...
      public function create_collection_with_split($title = 'Payment For Rent', $mode = '')
        {
            ...
            $split_payment =  array('email' => 'smartlever.sdn.bhd@gmail.com','fixed_cut' => 100, 'split_header' => false ,'company' => 'testCompany');   
        ...
        public function getURL()
        {
            $autoSubmit = '';
            if ($this->autoSubmit){
                $autoSubmit = '?auto_submit=true';
            }
            return $this->url . $autoSubmit;
        }
        ...
        public function getFPXBanks($mode = '')
        {
            if (empty($mode)) {
                $mode = $this->check_api_key();
            }

            $this->obj->setAction('GETFPXBANKS');
            $this->obj->setURL($mode);

            $data = $this->obj->curl_action()['banks'];
            return $data;
        }
        public function getOnlineFPXBanks($mode = '')
        {
            if (empty($mode)) {
                $mode = $this->check_api_key();
            }

            $data = $this->getFPXBanks($mode);
            $fpxbanks = array();
            foreach ($data as $banks){
                if ($banks['active']){
                    array_push($fpxbanks, $banks['name']);
                }
            }
        ...
        public function setAutoSubmit($status){
            if ($status){
...
class BillplzAction
    {
        ...
        public function setURL($mode, $id = '')
        {
            ...
            } else if ($this->action == 'GETFPXBANKS') {
                $this->url .= 'v3/fpx_banks';
        ...
        public function curl_action($data = '')
        {
            ...
            if ($this->action == 'DELETE') {
                $reqType = 'DELETE';
            } else if ($this->action == 'CHECK' || $this->action == 'GETCOLLECTIONINDEX' || $this->action == 'CHECKCOLLECTION' || $this->action == 'GETTRANSACTIONINDEX' || $this->action == 'GETFPXBANKS') {
                $reqType = 'GET';
            } else {
                $reqType = 'POST';
            }
